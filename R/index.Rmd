...
tmp <- lbl[varCode %in% names(gha), varCode]
setcolorder(gha, c(tmp, setdiff(names(gha), tmp)))
setkey(lbl, varCode)
write.dta(
  setattr(gha, "var.labels", lbl[names(gha), paste(varLabel, " (", unit, ")")]),
...
tmp <- lbl[varCode %in% names(tza), varCode]
setcolorder(tza, c(tmp, setdiff(names(tza), tmp)))
setkey(lbl, varCode)
write.dta(
  setattr(tza, "var.labels", lbl[names(tza), paste(varLabel, " (", unit, ")")]),
