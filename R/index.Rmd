...
# To save time we choose to manually generate these shares using quantities (not values)
...
eth[, .N, keyby=.(wave, strata)]
eth.pts <- SpatialPointsDataFrame(eth[!is.na(lon_dd_mod), .(lon_dd_mod, lat_dd_mod)], 
  data.frame(eth[!is.na(lon_dd_mod), .(survey, wave, svyCode, hhid, strata, region, district, ward)]),
  proj4string=CRS("+init=epsg:4326"))
...
eth %>%
  ...
  setattr("datalabel", "Prepared by BACOU, Melanie <mel@mbacou.com> for BMGF") %>%
  write.dta("./out/eth-ess_merged_2017.11.08.dta", version=12L, convert.factors="labels")
