...
This study combines household-level micro data for 4 countries across multiple survey years. To expedite all data reconciliation steps we choose to rely as much as possible on FAO/RIGA cross-country harmonized household-level database. RIGA expenditure and income aggregates are used to estimate both farm and crop sales and farm vs. non-farm income. We also use constructed variables from IFPRI replication datasets. Below is a full codebook.
...
for (i in unique(lbl$section)) {
  print(htmltools::h5(tools::toTitleCase(i)))
