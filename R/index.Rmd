...
theme_wc <- function(...) theme_bw(base_size=8, base_family="Roboto Condensed", ...) + 
  theme(panel.grid=element_line(linetype="dotted"))

#####################################################################################
# Helper - Broom results from svytotal(), svymean() and svyquantile()
#####################################################################################
#' Combine and format estimated totals and means from svyby()
#' @param f list of formulae (list(~var1, ~var2, ...)) to estimate each var separately
#' @param by passed to svyby()
#' @param design passed to svyby()
#' @param FUN passed to svyby()
#' @param ... other arguments passed to svyby()
#' @usage Use tables::tabular() to pivot results
svyCrossTab <- function(f, by, design, FUN=svymean, quantiles=.5, ...) {
...
#####################################################################################
# Helper - Broom survey proportions from svytotal() or svymean(~interaction())
#####################################################################################
#' Combine and format proportions estimated using svytotal/svymean(~interaction())
#' @param x character vector of factor variables whose proportions to estimate
#' @param design survey design passed to svytotal() and svymean()
#' @param prop "mean" for proportions or "total" for frequencies
#' @param ... other arguments passed to svytotal() and svymean()
svyCrossProp <- function(x, design, prop=c("mean", "total"), ...) {
  if(length(x) == 1) f <- formula(paste0("~", x))
  if(length(x) > 1) f <- formula(paste0("~I(interaction(", paste(x, collapse=", "), "))"))
  res <- switch(prop,
    mean = svymean(f, design, na.rm=TRUE, ...),
  ...
  res.cv <- confint(res)
  res <- switch(prop,
    mean = as.data.table(cbind(100*res, 100*res.cv)),
    total = as.data.table(cbind(res, res.cv))
  )
  setnames(res, c("mean", "CI1", "CI2"))
  res <- cbind(rn, res)
  res[CI1 < 0, CI1 := 0]
  res[, `:=`(
    CI1 = prettyNum(CI1, digits=switch(prop, mean=2, total=0), big.mark=","),
    CI2 = prettyNum(CI2, digits=switch(prop, mean=2, total=0), big.mark=",")
    )]
  res[, `:=`(
  ...
  setnames(res, "mean", switch(prop, mean="mean", total="total"))
  setnames(res, seq_along(x), x)
  for (i in x) res[[i]] <- factor(res[[i]], levels=levels(design$variables[[i]]))
  return(res)
}
...
class5 <- c("SUB", "TRANS", "PRCOMM", "SPCOMM", "DIV")
...
# Load BMGF/SDI ag. potential (basd on NDVI)
# Received from Todd on 2017.12.12
# Includes WorldPop density at 1km, mean of dekadal max NDVI, ESA CCI land cover
# ESA CCI might be a more recent version than 2015
# They look very large, verify
rndvi <- raster("./out/SDI/NDVI_1KM_ETH-GHA-TZA.tif")
res(rndvi)
proj4string(rndvi)
# [1] 0.008332501 0.008332178
rndvi <- crop(rndvi, g2, filename="./out/SDI/NDVI_MeanofDekadalMax_1km_cropped.tif")
rndvi <- raster("./out/SDI/NDVI_MeanofDekadalMax_1km_cropped.tif")
# => need to fix dekadal scale (I assume it's MODIS)
# Nil NDVI invalid values: 201 - 255 (hopefully done by Joe)
...
rndvi[] <- (rndvi[]-100)/100
spplot(rndvi)
writeRaster(rndvi, "./out/SDI/NDVI_MeanofDekadalMax_1km_cropped.tif", overwrite=TRUE)

rpop <- raster("~/Maps/Worldpop/AFR_PPP_2015_adj_v2.tif")
res(rpop)
proj4string(rpop)
# [1] 0.008333333 0.008333333
rpop <- crop(rpop, g2, filename="./out/SDI/AFR_PPP_2015_adj_v2_1km_cropped.tif")
rpop <- raster("./out/SDI/AFR_PPP_2015_adj_v2_1km_cropped.tif")
spplot(rpop)

rlc <- raster("~/Maps/ESA/ESACCI-LC-1km_ETH-GHA-TZA.tif")
# Should be coded 0-220 (make sure mode was used)
res(rlc)
# [1] 0.0001851853 0.0001851854
# wrong resolution, correct
rlc <- raster("~/Maps/ESA/ESACCI-LC-L4-LCCS-Map-300m-P1Y-2015-v2.0.7.tif")
res(rlc)
# [1] 0.002777778 0.002777778
rlc <- crop(rlc, g2)
rlc[rlc==0] <- NA
rlc <- aggregate(rlc, fact=res(rpop)/res(rlc)[1], fun=modal, na.rm=TRUE, filename="./out/SDI/ESACCI-LC-L4-LCCS-Map-1km-P1Y-2015-v2.0.7.tif")
rlc <- raster("./out/SDI/ESACCI-LC-L4-LCCS-Map-1km-P1Y-2015-v2.0.7.tif")
spplot(rlc)
...
m <- tm_shape(rndvi) + 
  tm_raster(pal=pal.ndvi(255), breaks=seq(0,1,0.1),
  title="Long-term\nNDVI\nmean of \ndekadal\nmaxima\n(index)") +
...
m <- tm_shape(rlc) + 
  tm_raster(
    pal=qd.lc.lbl[J(unique(rlc)), col], 
    breaks=c(-1, unique(rlc)),
    labels=qd.lc.lbl[J(unique(rlc)), cat],
    title="ESA Land Cover\nClassification") +
  tm_shape(g2[g2$svyCode %in% i,], is.master=T) + tm_borders(alpha=.1) +
  tm_facets("ISO3", ncol=3, free.coords=T)

save_tmap(m, "./docs/out/esa-lc.png", width=6, height=3.6)

# WorldPop
m <- tm_shape(rpop) + 
  tm_raster(pal="YlOrRd", breaks=seq(0, 90, 10),
  title="WorldPop\nPopulation\nDensity\n(pp/sq. km.)") +
  tm_shape(g2[g2$svyCode %in% i,], is.master=T) + tm_borders(alpha=.1) +
  tm_facets("ISO3", ncol=3, free.coords=T)

save_tmap(m, "./docs/out/wpop-dens.png", width=6, height=3.6)

###############################################
# Import new quadrant classification vars from WorldBank
# Received from <smurray@worldbank.org> on 2018.01.18
# We decided to combine long-term NDVI and pop density at 1km instead of using GAEZ
# Also using ESA land cover mask
# Long-term max NDVI was prepared by IFPRI (from MODIS)

# Load latest WorldBank spatial codes (NDVI, pop density, ESA land cover)
f <- list.files("./out/WB/2018.01.19/", "*.csv", full.names=T)
wb2 <- lapply(f, fread)
wb2 <- lapply(wb2, setnames, c("hhid", "lc_esa_v207", "ndvi", "popdens_2015"))
wb2 <- lapply(wb2, function(x) x[, hhid := as.character(hhid)])
names(wb2) <- c("eth-erss-2011", "eth-ess-2013", "eth-ess-2015",
  "tza-nps-2008", "tza-nps-2010", "tza-nps-2012")
wb2 <- rbindlist(wb2, idcol="svyCode")

# Add NDVI, ESA, and pop density across Ghana districts for all 3 waves
...
# set croparea_ha==0 for hhlds with aggross > 0
gha[, sum(aggross>0), by=croparea==0]
#    croparea    V1
# 1:     TRUE  6086
# 2:    FALSE 10679
# OK we have hhlds with aggross but no land
...
gha[, croparea_4ha := factor(croparea_imp <= 4, levels=c(T, F), labels=c("<= 4 ha", "> 4 ha"))]
...
gha[, croparea_3clas := NULL]
gha[, max(croparea_imp, na.rm=T)]
gha[, croparea_3clas := cut(croparea_imp, c(-1,2,4,106), labels=c("<= 2 ha", "2-4 ha", "> 4 ha"), ordered=T, right=F)]
summary(gha$croparea_3clas)
# <= 2 ha  2-4 ha  > 4 ha 
#   22642    5503    3312  
...
gha[, class5 := class6]
levels(gha$class5) <- class5[c(1,2,3,2,4,5)]
summary(gha$class5)
#  SUB  TRANS PRCOMM SPCOMM    DIV   NA's 
...
# set croparea_ha==0 for hhlds with aggross > 0
tza[, sum(aggross>0), by=!is.na(croparea_imp)]
#    is.na   V1
# 1:  TRUE 7606
tza[, sum(aggross>0), by=croparea_imp==0]
#    croparea   V1
# 1:    FALSE 7292
# 2:     TRUE  314
# OK we have hhlds with aggross but no land
...
tza[, croparea_4ha := factor(croparea_imp <= 4, levels=c(T, F), labels=c("<= 4 ha", "> 4 ha"))]
...
tza[, max(croparea_imp, na.rm=T)]
tza[, croparea_3clas := cut(croparea_imp, c(-1,2,4,51), labels=c("<= 2 ha", "2-4 ha", "> 4 ha"), ordered=T, right=F)]
...
# Farm classification dummy `class5`
tza[, class5 := class6]
levels(tza$class5) <- class5[c(1,2,3,2,4,5)]
c("subs.", "trans.", "pre-comm.", "trans.", "specd. comm.", "divf. comm.")
summary(tza$class5)
#  SUB  TRANS PRCOMM SPCOMM    DIV   NA's 
...
# set croparea_ha==0 for hhlds with aggross > 0
eth[, sum(aggross>0), by=!is.na(croparea_imp)]
#    is.na   V1
# 1:  TRUE 7606
eth[, sum(aggross>0), by=croparea_imp==0]
#    croparea_imp   V1
# 1:        FALSE 5911
# 2:         TRUE 1184
# OK we have hhlds with aggross but no land
...
eth[, croparea_4ha := factor(croparea_imp <= 4, levels=c(T, F), labels=c("<= 4 ha", "> 4 ha"))]
...
eth[, max(croparea_imp, na.rm=T)]
eth[, croparea_3clas := cut(croparea_imp, c(-1,2,4,12), labels=c("<= 2 ha", "2-4 ha", "> 4 ha"), ordered=T, right=F)]
...
##########################################
# Crop commercialization dummy `cropsales_3clas`
# BEWARE we can use `shcropsold` or use `cropsales`_sh
eth[, cropsales_3clas := NULL]
eth[, cropsales_3clas := cut(shcropsold, c(-1,.05,.5,1.1), labels=c("LoC", "MeC", "HiC"), ordered=T, right=F)]
...
# Farm classification dummy `class5`
eth[, class5 := class6]
levels(eth$class5) <- class5[c(1,2,3,2,4,5)]
summary(eth$class5)
# SUB  TRANS PRCOMM SPCOMM    DIV   NA's 
# 820    442   2100   1249    147   4339 

# Verify sampling designs (defined below)
svytotal(~hhsize, eth.svy[["eth2"]])
#           total      SE
# hhsize 80768450 2829245
# => OK
...
#####################################################################################
# Define "farms" households
#####################################################################################

gha[, farm := FALSE]
gha[aggross > 0 | croparea_imp > 0, farm := TRUE]

tza[, farm := FALSE]
tza[aggross > 0 | croparea_imp > 0, farm := TRUE]

eth[, farm := FALSE]
eth[aggross > 0 | croparea_imp > 0, farm := TRUE]
...
# Subset survey sample to farms (this includes croparea==0 & aggross > 0)
gha.svy.farm <- lapply(gha.svy, subset, farm==TRUE)

# Subset survey sample to SHF
gha.svy.shf <- lapply(gha.svy.farm, subset, croparea_4ha=="<= 4 ha")
...
tza.svy.farm <- lapply(tza.svy, subset, farm==TRUE)
tza.svy.shf <- lapply(tza.svy.farm, subset, croparea_4ha=="<= 4 ha")
...
eth.svy.farm <- lapply(eth.svy, subset, farm==TRUE)
eth.svy.shf <- lapply(eth.svy.farm, subset, croparea_4ha=="<= 4 ha")

```

```{r save, eval=FALSE}

rm(tmp, tmp1, x, y, i, j, tza.adm, tza.chr, tza.inc, tza.hh, m,
  eth.adm, eth.chr, eth.inc, eth.hh, tmp.cv, f, by, design, rn, res, res.cv)
...
gha %>%
  ...
  write.dta("./out/gha-glss_merged_2017.12.05.dta", version=12L, convert.factors="labels")
...
tza %>%
  ...
  setattr("datalabel", "Prepared by BACOU, Melanie <mel@mbacou.com> for BMGF") %>%
  write.dta("./out/tza-nps_merged_2017.12.05.dta", version=12L, convert.factors="labels")
...
eth %>%
  ...
  setattr("datalabel", "Prepared by BACOU, Melanie <mel@mbacou.com> for BMGF") %>%
  write.dta("./out/eth-ess_merged_2017.12.05.dta", version=12L, convert.factors="labels")
...
In a second approach we revise the definition of agricultural potential quadrants and combine long-term NDVI (MODIS) with population density (both rasters are available at 1km spatial resolution).

```{r maps-ndvi, fig.cap="Long Term NDVI - Mean of Dekadal Maxima", out.width="99%"}

tm_shape(crop(rndvi, g2[g2$svyCode %in% i,])) + tm_raster(pal=pal.ndvi(19),
  title="Long-term\nNDVI") +
  tm_shape(g2[g2$svyCode %in% i,], is.master=T) + tm_borders(alpha=.1) +
  tm_facets("ISO3", ncol=3, free.coords=T)
