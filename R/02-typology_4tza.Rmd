...
par(mfrow=c(1,3), mar=c(3,0.25,2,.25))
plot(svytable(~seg_quad+class5, tza.svy.shf[["tza3"]]), main=NA, las=2, col=viridis(5),
  ylab=NA, xlab="5-types and Quadrants (2012/13)")
plot(svytable(~seg_quad+class5, tza.svy.shf[["tza2"]]), main=NA, las=2, col=viridis(5),
  ylab=NA, xlab="5-types and Quadrants (2010/11)")
plot(svytable(~seg_quad+class5, tza.svy.shf[["tza1"]]), main=NA, las=2, col=viridis(5),
  ylab=NA, xlab="5-types and Quadrants (2008/09)")
...
# NPS3
tmp <- svymean(~I(interaction(seg_quad, class5)), tza.svy.shf[["tza3"]], na.rm=T)
tmp.cv <- confint(tmp)
tmp <- cbind(as.data.table(tmp*100), as.data.table(tmp.cv*100))
tmp[`2.5 %`<0, `2.5 %` := 0]
tmp <- tmp[, lapply(.SD, function(x) format(x, digits=0, nsmall=1, scientific=F))]
...
# NPS2
tmp <- svymean(~I(interaction(seg_quad, class5)), tza.svy.shf[["tza2"]], na.rm=T)
tmp.cv <- confint(tmp)
tmp <- cbind(as.data.table(tmp*100), as.data.table(tmp.cv*100))
tmp[`2.5 %`<0, `2.5 %` := 0]
tmp <- tmp[, lapply(.SD, function(x) format(x, digits=0, nsmall=1, scientific=F))]
...
# NPS1
tmp <- svymean(~I(interaction(seg_quad, class5)), tza.svy.shf[["tza1"]], na.rm=T)
tmp.cv <- confint(tmp)
tmp <- cbind(as.data.table(tmp*100), as.data.table(tmp.cv*100))
tmp[`2.5 %`<0, `2.5 %` := 0]
tmp <- tmp[, lapply(.SD, function(x) format(x, digits=0, nsmall=1, scientific=F))]
